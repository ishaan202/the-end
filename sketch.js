...
function draw() {
  ...
  if(gameState === play) {
    ...
    text("BECAUSE OF YOU!!YOU SHOULD HAVE BEEN. ",canvas.width/2-400,canvas.height/2+150)
    text("Your final score:"+score,canvas.width/2-400,canvas.height/2+250);
    text("CAREFULL!!!!!!!!!!",canvas.width/2-400,canvas.height/2+200);
  ...
  if(score == 100){
    ...
    fill("black");
    textSize(40);
    text("You protected your planet from viruses.",canvas.width/2-350,canvas.height/120);
    text("you are now the hero of the planet.YAYYYY!!!!!!",canvas.width/2-350,canvas.height/160);
    text("You Win!!!NOW THERE WILL BE STATUES BUILT IN HONOR OF YOU!!",canvas.width/2-70,canvas.height/2+60);
